...
def d3_link_to_arango_doc(link: Dict, node_table_name: str) -> Dict:
    ...
    # Check if we have a field we can use for from and to. _from and _to are preferred, then source and target
...
def process_d3_json(
    ...
    if 'links' in d3_dict.keys():
        ...
        raise DataFormatException("JSON network file missing 'links' or 'edges' property")

    d3_dict[link_property_name] = [
        link
        for link in (d3_link_to_arango_doc(link, node_table_name) for link in d3_dict[link_property_name])
        if link is not None
    ]
